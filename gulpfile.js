...
gulp.task('build-js', function() {
    var vendorFiles = [basePaths.bower + 'jquery/dist/jquery.min.js',
                       basePaths.bower + 'jQuery-viewport-checker/dist/jquery.viewportchecker.min.js',
                       basePaths.bower + 'material-design-lite/material.min.js'],
...
gulp.task('build-css', function() {
    ...
    var vendorFiles = gulp.src(basePaths.bower+'animate.css/animate.css'), //components
...
gulp.task('rev-html', function() {
    ...
    return gulp.src([basePaths.dest+'rev/*.json', '*.html'])
        .pipe( revCol({
            replaceReved: true,
            dirReplacements: {
                'assets/css': 'assets/rev',
                'assets/js' : 'assets/rev'                
            }
        }) )        
        .pipe( gulp.dest('.') )
        .on('error', console.log); //log  
...
gulp.task('svg-index', function () {
      
    return  gulp.src(basePaths.src +'html/index.html')
        .pipe(fileInc({
            prefix   : '@@',
            basepath : '@root'
        }))        
...
gulp.task('full-build-svg', function(callback) {
    runSequence('svg-opt',
        'svg-index',             
        callback);
});
...
gulp.task('watch', function(){
    ...
    gulp.watch(basePaths.src+'html/*.html', ['svg-index']).on('change', function(evt) {
        changeEvent(evt);
    });
});
