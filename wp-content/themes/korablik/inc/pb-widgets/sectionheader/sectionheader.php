...
class IST_PB_SectionHeader_Widget extends SiteOrigin_Widget {
	...
	function initialize_form(){
		...
		return array(
			'title' => array(
				'type' => 'text',
				'label' => __('Title text', 'krbl'),
			),
			
			'subtitle' => array(
				'type' => 'text',
				'label' => __('Sub title text', 'krbl'),				
			),
			...
			'page_header' => array(
				'type' => 'checkbox',
				'label' => __( 'Style as page title', 'krbl' ),
				'default' => false
			)
		);
	}
	...
	<div class="pb-section-title <?php echo esc_attr($css_title); ?>">	
		<h3><?php echo apply_filters('krbl_the_title', $title);?></h3>
		<?php if($subtitle) { ?>
			<h4><?php echo apply_filters('krbl_the_title', $subtitle); ?></h4>
		<?php } ?>		
	</div>
	<?php	
		echo '</div>';
