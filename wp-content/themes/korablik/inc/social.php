...
add_filter( 'pre_wp_nav_menu', 'krbl_pre_wp_nav_menu_social', 10, 2 );
function krbl_pre_wp_nav_menu_social( $output, $args ) {
	...
	$supported_icons = apply_filters( 'krbl_supported_social_icons', array(
...
foreach($data as $key => $obj){		
	if((krbl_is_mobile_user_agent() && $obj['show_mobile']) || !krbl_is_mobile_user_agent()){
?>
...
<?php 
	...
	foreach($mobile as $key => $obj) {
		...
		if((!krbl_is_mobile_user_agent() && $obj['show_desktop']) || krbl_is_mobile_user_agent()) {
?>
	<div title="<?php echo esc_attr($obj['label']);?>" class="social-likes__widget social-likes__widget_<?php echo $key;?>">
...
function krbl_is_mobile_user_agent(){
...
add_shortcode('newsletter_form', 'krbl_newsletter_form_screen');
function krbl_newsletter_form_screen($atts){
	
	return "<div class='newsletter-form'>".krbl_get_newsletter_form()."</div>";
}
