...
$format = krbl_get_post_format($cpost);
$video = $thumbnail = '';
...
<div class="container">
	<header class="entry-header-full">
		<div class="entry-meta"><?php echo krbl_posted_on($cpost); //for event ?></div>
	...
	<div class="frame">
		...
		<?php if($format == 'standard') { ?>
			<div class="entry-preview">
				<?php echo krbl_post_thumbnail($cpost->ID, 'medium-thumbnail');?>						
			</div>
		...
		<?php } ?>				
			...
			<div class="entry-content">
				<div class="lead"><?php echo apply_filters('krbl_the_content', $cpost->post_excerpt); ?></div>
				<?php echo apply_filters('the_content', $cpost->post_content); ?>
			</div>
		</main>
		
		<div id="krbl_sidebar" class="bit md-4">
		<?php
			//$rquery = krbl_get_related_query($cpost, 'post_tag', 1); 
			//if($rquery->have_posts()){
			//	krbl_related_project($rquery->posts[0]);
			//}
...
<?php
	if($cpost->post_type == 'post') {
		...
		krbl_more_section($pquery->posts, __('Related news', 'krbl'), 'news', 'addon'); 
	...
	elseif($cpost->post_type == 'project') {
		...
		if($pquery->have_posts()){
			krbl_more_section($pquery->posts, __('Related projects', 'krbl'), 'projects', 'addon'); 
		}
	}
		...
		$pquery = new WP_Query(array(
			...
			'tax_query' => array(
				array(
					'taxonomy' => 'person_cat',
					'field' => 'id',
					'terms' => (!empty($cat)) ? krbl_get_term_id_from_terms($cat): array()
				)
			)
		));
		
		if($pquery->have_posts()){
			krbl_more_section($pquery->posts, __('Our volunteers', 'krbl'), 'people', 'addon'); 
		}
	}
