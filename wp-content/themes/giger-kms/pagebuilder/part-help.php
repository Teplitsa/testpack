...
$support_id = get_queried_object_id();
$volunteer_id = get_queried_object_id();
$corporate_id = get_queried_object_id();
...
function tst_linked_help_card($help_id, $img_id = 0, $args = array()){
	...
	if($img_id > 0){
		...
		$thumbnail = get_the_post_thumbnail($help_id, $args['size']);
	}
?>
<a href="<?php echo $pl;?>" class="card-link">
	...
	<div class="help-block__content">
		<div class="flex-grid--stacked <?php echo $grid_css;?>">
			<div class="flex-cell--stacked sm-12 lg-6 card card--linked block-2col">
				<?php tst_linked_help_card($support_id, (int)$img1, array('size' => 'block-2col')) ;?>
			</div>
			<div class="flex-cell--stacked sm-6 lg-3 card card--colored">
				<?php tst_colored_help_card($volunteer_id) ;?>
			</div>
			<div class="flex-cell--stacked sm-6 lg-3 card card--item">
				<?php tst_linked_help_card($corporate_id, (int)$img2, array('size' => 'block-small', 'title' => __('Corporate help', 'tst'), 'button' => __('Become partner', 'tst'))) ;?>
			</div>
		</div>
	</div>
</div>