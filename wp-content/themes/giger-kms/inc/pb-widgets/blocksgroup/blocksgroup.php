...
class TST_BlocksGroup_Widget extends SiteOrigin_Widget {
	...
	public function widget( $args, $instance ) {
		...
		$widget_class = (isset($instance['panels_info']['style']['class'])) ? $instance['panels_info']['style']['class'] : '';
		$loop_css = "cards-loop sm-cols-2 md-cols-3 lg-cols-4 exlg-cols-5";
		
		if($instance['post_type'] == 'programm'){
			$loop_css = (false !== strpos($widget_class, 'compact')) ?  "cards-loop sm-cols-2 md-cols-2 lg-cols-4" : "cards-loop sm-cols-2 md-cols-2";			
		}
		...
		?>
			<div class="tst-pb-blocks"><div class="<?php echo $loop_css;?>">
				<?php foreach ($posts as $p) {					
					$p->widget_class = ($widget_class) ? $widget_class : 'default';
