...
if( !function_exists('tst_get_default_map_markers') ) {
    function tst_get_default_map_markers() {
        ...
        $result = get_transient('default_ecoproblems_map_markers');
        if( !$result ) {
            ...
            set_transient('default_ecoproblems_map_markers', $result);
...
//    add_action('save_post', 'tst_invalidate_homepage_blocks');
    ...
    function tst_invalidate_map_markers($marker_id){
        ...
        if( !get_transient('default_ecoproblems_map_markers') ) {
            set_transient('default_ecoproblems_map_markers', get_posts(array(
                'post_type' => 'marker',
                'posts_per_page' => -1,
                'orderby'   => 'meta_value',
                'meta_key'  => 'marker_city',
