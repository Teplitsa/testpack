...
add_filter('manage_posts_columns', 'kds_common_columns_names', 50, 2);
function kds_common_columns_names($columns, $post_type) {
...
add_action('manage_pages_custom_column', 'kds_common_columns_content', 2, 2);
add_action('manage_posts_custom_column', 'kds_common_columns_content', 2, 2);
function kds_common_columns_content($column_name, $post_id) {
...
add_filter('manage_pages_columns', 'kds_pages_columns_names', 50);
function kds_pages_columns_names($columns) {		
...
add_filter( "manage_edit-category_columns", 'kds_common_tax_columns_names', 10);
add_filter( "manage_edit-post_tag_columns", 'kds_common_tax_columns_names', 10);
function kds_common_tax_columns_names($columns){
...
add_filter( "manage_category_custom_column", 'kds_common_tax_columns_content', 10, 3);
add_filter( "manage_post_tag_custom_column", 'kds_common_tax_columns_content', 10, 3);
function kds_common_tax_columns_content($content, $column_name, $term_id){
...
add_action('admin_init', function(){
	foreach(get_post_types(array('public' => true), 'names') as $pt) {
		add_filter('manage_' . $pt . '_posts_columns', 'kds_clear_seo_columns', 100);
	}	
}, 100);

function kds_clear_seo_columns($columns){
...
/* Excerpt metabox */
add_action('add_meta_boxes', 'kds_correct_metaboxes', 2, 2);
function kds_correct_metaboxes($post_type, $post ){
	
	if(post_type_supports($post_type, 'excerpt')){
		remove_meta_box('postexcerpt', null, 'normal');
		
		$label = ($post_type == 'org') ? __('Website', 'kds') : __('Excerpt', 'kds');
		add_meta_box('kds_postexcerpt', $label, 'kds_excerpt_meta_box', null, 'normal', 'core');
	}
	
}

function kds_excerpt_meta_box($post){
...
add_action('admin_menu', 'kds_add_menu_items', 25);
function kds_add_menu_items(){
...
/** Visual editor **/
add_filter('tiny_mce_before_init', 'kds_format_TinyMCE');
function kds_format_TinyMCE($in){
...
/* Menu Labels */
add_action('admin_menu', 'kds_admin_menu_labels');
function kds_admin_menu_labels(){ /* change adming menu labels */
    global $menu, $submenu;
...
add_filter('admin_post_thumbnail_html', 'kds_thumbnail_dimensions_check', 10, 2);
function kds_thumbnail_dimensions_check($thumbnail_html, $post_id) {
...
//add_action( 'add_meta_boxes' , 'kds_remove_wrong_metaboxes', 20 );
function kds_remove_wrong_metaboxes() {
