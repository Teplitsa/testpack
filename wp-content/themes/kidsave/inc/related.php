...
function kds_related_posts_rules(){
...
/* get relalted ids */
function kds_get_related_ids($cpost, $tax = 'post_tag', $limit = 5){
	global $wpdb, $related_pt_rules;
		
	$related_ids = array();
	
	//params
	$related_pt_rules = kds_related_posts_rules();
	$post_type = (isset($related_pt_rules[$cpost->post_type])) ? $related_pt_rules[$cpost->post_type] : '';
	$post_type = apply_filters('kds_related_post_types', $post_type, $cpost, $tax); //sometimes we need to alter it outside
...
/* build related query */
function kds_get_related_query($cpost, $tax = 'post_tag', $limit = 5) {
	
	if(empty($cpost))
		$cpost = get_post();
	
	
	$r_ids = kds_get_related_ids($cpost, $tax, $limit); 
