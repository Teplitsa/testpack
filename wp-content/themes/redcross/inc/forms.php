...
add_action('after_setup_theme', 'rdc_correct_forms_display');
function rdc_correct_forms_display(){
	
	remove_shortcode('vfb');
	add_shortcode('vfb', 'rdc_vfb_form_screen');
}

function rdc_vfb_form_screen($atts) {
	global $wpdb, $visual_form_builder;
...
//if ( !$visual_form_builder->add_scripts )
	$visual_form_builder->scripts();
...
if ( isset( $_POST['vfb-submit'] ) && isset( $_POST['form_id'] ) && $_POST['form_id'] == $form_id ) {
	$output = $visual_form_builder->confirmation();
	return $output;
}

$order = sanitize_sql_orderby( 'form_id DESC' );
$form  = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $visual_form_builder->form_table_name WHERE form_id = %d ORDER BY $order", $form_id ) );
...
$fields         = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $visual_form_builder->field_table_name WHERE form_id = %d ORDER BY $order_fields", $form_id ) );
